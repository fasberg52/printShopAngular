...
import { PriceRule, MatchCondition } from '../models/price-rule.model';
...
export class PriceCalculationService {
   ...
   */
  calculatePrice(specs: ProductSpecs): Observable<PriceCalculationResult | null> {
    return this.priceRuleService.getAllPriceRules().pipe(
      map(priceRules => {
        ...
        const calculation = this.calculatePriceFromRule(applicableRule, specs.quantity);
   ...
   */
  private findApplicableRule(priceRules: PriceRule[], specs: ProductSpecs): PriceRule | null {
    // Filter active rules for the product type
    const activeRules = priceRules.filter(rule =>
      rule.isActive && rule.productType === specs.productType
    );
   ...
   */
  private ruleMatchesConditions(rule: PriceRule, attributes: Record<string, string>): boolean {
   ...
   */
  private calculatePriceFromRule(rule: PriceRule, quantity: number): {
    unitPrice: number;
    totalPrice: number;
   ...
   */
  getPriceMatrix(productType: 'print' | 'binding'): Observable<PriceCalculationResult[]> {
    return this.priceRuleService.getAllPriceRules().pipe(
      map(priceRules => {
        const activeRules = priceRules.filter(rule =>
          rule.isActive && rule.productType === productType
        );
      ...
      map(result => {
        if (result) {
          ...
          return {
            ...
            suggestion: 'لطفا قوانین قیمت‌گذاری را برای این نوع محصول بررسی کنید'
          };
        }
      })
    );
  ...
  getRuleSuggestions(specs: ProductSpecs): Observable<string[]> {
    return this.priceRuleService.getAllPriceRules().pipe(
      map(priceRules => {
        ...
        for (const rule of activeRules) {
          const missingAttributes = rule.match.filter(condition =>
            !specs.attributes[condition.attributeKey]
          );
          ...
          if (missingAttributes.length > 0) {
            ...
            suggestions.push(`قانون "${rule.name}" نیاز به مشخصات ${missingNames.join(' و ')} دارد`);
          }
        }
