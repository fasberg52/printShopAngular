...
@Component({
  ...
  animations: [
    ...
    trigger('filterAnimation', [
      ...
      transition(':leave', [
        animate(
          '300ms cubic-bezier(0.55, 0, 0.68, 0.53)',
          style({ opacity: 0, transform: 'translateY(-20px) scale(0.95)' })
        ),
      ]),
    ]),
...
export class PricingDisplayComponent implements OnInit {
  ...
  productVariants = computed<ProductVariant[]>(() => {
    ...
    // A4 variants
    this.addPrintVariant(
      variants,
      printRules,
      'A4 تک‌رو سیاه و سفید',
      'a4',
      'blackAndWhite',
      'single_sided'
    );
    ...
    this.addBindingVariant(
      variants,
      bindingRules,
      'صحافی فنری معمولی A4',
      'springNormal',
      'a4'
    );
    this.addBindingVariant(
  ...
  printVariants = computed<ProductVariant[]>(() => {
    let variants = this.productVariants().filter(
      (v) => v.productType === 'print'
    );

    // Apply size filter
    if (this.selectedSize() !== 'all') {
      variants = variants.filter(
        (v) => v.attributes['size'] === this.selectedSize()
      );
    }
    ...
    if (this.selectedColor() !== 'all') {
      variants = variants.filter(
        (v) => v.attributes['color'] === this.selectedColor()
      );
    }
  ...
  filteredVariants = computed<ProductVariant[]>(() => {
    ...
    if (this.selectedProductType() !== 'all') {
      variants = variants.filter(
        (v) => v.productType === this.selectedProductType()
      );
    }
    ...
    if (
      ...
      this.selectedProductType() !== 'binding'
    ) {
      variants = variants.filter(
        (v) => v.attributes['color'] === this.selectedColor()
      );
  ...
  hasBindingVariants = computed<boolean>(
    () => this.bindingVariants().length > 0
  );
  hasFilteredVariants = computed<boolean>(
    () => this.filteredVariants().length > 0
  );

  // Animation states for sections
  printSectionState = computed<string>(() =>
    this.hasPrintVariants() ? 'visible' : 'hidden'
  );
  bindingSectionState = computed<string>(() =>
    this.hasBindingVariants() ? 'visible' : 'hidden'
  );

  // Computed signals for template filtering
  hasPrintInFiltered = computed<boolean>(
    () =>
      this.filteredVariants().filter((v) => v.productType === 'print').length >
      0
  );
  hasBindingInFiltered = computed<boolean>(
    () =>
      this.filteredVariants().filter((v) => v.productType === 'binding')
        .length > 0
  );
  ...
  shouldShowVariant(variant: ProductVariant): boolean {
    ...
    if (
      ...
      variant.productType !== this.selectedProductType()
    ) {
      return false;
    }
    ...
    if (
      ...
      variant.attributes['size'] !== this.selectedSize()
    ) {
      return false;
    }
    ...
    if (
      this.selectedColor() !== 'all' &&
      variant.productType === 'print' &&
      variant.attributes['color'] !== this.selectedColor()
    ) {
      return false;
    }
  ...
  private addBindingVariant(
    ...
    const rule = rules.find(
      (r) =>
        r.match.some(
          (m) => m.attributeKey === 'bindingType' && m.value === bindingType
        ) && r.match.some((m) => m.attributeKey === 'size' && m.value === size)
    );
  ...
  private calculatePrices(
    rule: PriceRule
  ): { range: string; quantity: number; price: number }[] {
    const prices: { range: string; quantity: number; price: number }[] = [];

    // Base price (1 to first breakpoint or unlimited)
    const firstBreakpoint =
      rule.breakpoints.length > 0 ? rule.breakpoints[0].at : null;
    prices.push({
      range: firstBreakpoint
        ? `۱-${firstBreakpoint - 1} برگه`
        : '۱ برگه به بالا',
      quantity: 1,
      price: rule.price,
    });
